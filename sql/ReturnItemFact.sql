SELECT
  ...
  (
    ...
    FROM return_adjustment ra
    WHERE ra.RETURN_ID = ri.RETURN_ID
      AND ra.RETURN_ADJUSTMENT_TYPE_ID = 'SHPFI_REFUND_DISCREP'
  ) AS `SHPFI_REFUND_DISCREP`,
  (
    SELECT SUM(ra.AMOUNT)
    FROM return_adjustment ra
    WHERE ra.RETURN_ID = ri.RETURN_ID
      AND ra.RETURN_ADJUSTMENT_TYPE_ID = 'SHPFI_CUSTOMER'
  ) AS `SHPFI_CUSTOMER`,
  (
    SELECT SUM(ra.AMOUNT)
    FROM return_adjustment ra
    WHERE ra.RETURN_ID = ri.RETURN_ID
      AND ra.RETURN_ADJUSTMENT_TYPE_ID = 'SHPFI_DAMAGE'
  ) AS `SHPFI_DAMAGE`,
  (
    SELECT SUM(ra.AMOUNT)
    FROM return_adjustment ra
    WHERE ra.RETURN_ID = ri.RETURN_ID
      AND ra.RETURN_ADJUSTMENT_TYPE_ID = 'SHPFI_BALANCING_ADJ'
  ) AS `SHPFI_BALANCING_ADJ`,
  (
    SELECT SUM(ra.AMOUNT)
    FROM return_adjustment ra
    WHERE ra.RETURN_ID = ri.RETURN_ID
      AND ra.RETURN_ADJUSTMENT_TYPE_ID = 'SHPFI_PEND_REF_DISC'
  ) AS `SHPFI_PEND_REF_DISC`,
  (
    SELECT SUM(ra.AMOUNT)
    FROM return_adjustment ra
    WHERE ra.RETURN_ID = ri.RETURN_ID
      AND ra.RETURN_ADJUSTMENT_TYPE_ID = 'SHPFI_RESTOCK'
  ) AS `SHPFI_RESTOCK`,
  ...
  rs.CREATED_STAMP AS 'cursorDate'
FROM return_item ri
JOIN return_header rh ON rh.RETURN_ID = ri.RETURN_ID AND rh.RETURN_HEADER_TYPE_ID!="APPEASEMENT"
